...
public class AwsCognitoService {
    ...
    /**
     * Registra al usuario y lo auto‑confirma, y devuelve el 'sub' que Cognito
     * asigna.
     * 
     * @param username
     * @param password
    ...
    public ChangePasswordResult changePassword(String accessToken, String oldPassword, String newPassword) {
        if (accessToken == null || accessToken.isBlank()) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Token de acceso requerido");
        }
        if (oldPassword == null || oldPassword.isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "La contraseña actual no puede estar vacía");
        }
        if (newPassword == null || newPassword.isBlank()) {
        ...
        try {
            return cognitoClient.changePassword(new ChangePasswordRequest()
                    .withAccessToken(accessToken)
                    .withPreviousPassword(oldPassword)
                    .withProposedPassword(newPassword));
        } catch (NotAuthorizedException e) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED,
                    "Contraseña actual incorrecta o token inválido");
        } catch (InvalidPasswordException e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Contraseña inválida: debe tener minimo 8 caracteres conteniendo 1 numero, 1 caracter especial, 1 letra mayuscula y 1 letra minuscula");
        } catch (LimitExceededException e) {
            ...
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "Debes restablecer tu contraseña antes de poder cambiarla");
        } catch (SdkClientException e) {
            System.err.println(">>> COGNITO ERROR CAUSE: " + e.getMessage());
            e.printStackTrace();
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,
                    "Error de comunicación con AWS Cognito", e);
        } catch (Exception e) {
